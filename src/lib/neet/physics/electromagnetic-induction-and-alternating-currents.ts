...
*   **Faraday's law:** The magnitude of the induced EMF in a circuit is equal to the time rate of change of magnetic flux through the circuit. ε = -dΦ_B/dt.
*   **Induced emf and current; Lenz’s Law:** The direction of the induced current is such that it opposes the change that produced it.
*   **Eddy currents (சுழல் மின்னோட்டங்கள்):** Circulating currents induced in bulk pieces of conductors when the magnetic flux linked with them changes.

### 2. Inductance (தூண்டல்)
*   **Self and mutual inductance:** Self-inductance is the property of a coil to oppose a change in current in itself. Mutual inductance is the property to oppose a change in current in a neighboring coil.

### 3. Alternating Currents (மாறுதிசை மின்னோட்டம்)
*   **Alternating currents, peak and RMS value of alternating current/ voltage.** I_rms = I₀/√2, V_rms = V₀/√2.
*   **Reactance and impedance (மறுப்பு மற்றும் மின் எதிர்ப்பு):**
    *   Inductive Reactance (X_L) = ωL.
    *   Capacitive Reactance (X_C) = 1/ωC.
    *   Impedance (Z) = √(R² + (X_L - X_C)²).
*   **LCR series circuit, resonance:** A condition where the current is maximum (X_L = X_C).
*   **Power in AC circuits, wattless current:** P_avg = V_rms I_rms cos(φ), where cos(φ) is the power factor. Wattless current flows when the power factor is zero (purely inductive or capacitive circuit).
*   **AC generator and transformer (மின்மாற்றி).**
`,
  workedExamples: [
    {
  ...
  matchTheColumns: [
    ...
    },
    {
        ...
        column2: ["i) I²R heat loss in windings", "ii) Use of soft iron core", "iii) Superconductors", "iv) Used in galvanometer brakes"],
        answer: "a-iii, b-iv, c-ii, d-i"
    },
    {
