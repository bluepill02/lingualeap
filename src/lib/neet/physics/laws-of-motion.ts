...
3.  **Identify and Draw All Forces:** Go through a checklist of forces. For each force, draw a vector arrow originating from your point, pointing in the correct direction.
    *   **Non-Contact Force (தொடா விசை):** Does gravity (ஈர்ப்பு விசை) act on it? (Almost always, draw <b>W = mg</b> acting vertically downwards).
...
Since momentum <b>p = mv</b>, if mass 'm' is constant, this simplifies to the famous equation:
\`\\vec{F}_{net} = m\\vec{a}\`
This is a vector equation. It can be broken down into components: <b>ΣF_x = ma_x</b> and <b>ΣF_y = ma_y</b>. The core of problem-solving is applying these component equations after drawing an FBD.

#### Third Law (Action-Reaction - வினை மற்றும் எதிர்வினை)
For every action, there is an equal and opposite reaction.
*   Forces always occur in pairs (<b>F_AB = -F_BA</b>).
...
*   **Magnitude:** <b>F_p = ma₀</b>, where 'm' is the mass of the object being observed, and 'a₀' is the acceleration of the frame.
*   **Direction:** It acts in the direction **opposite** to the acceleration of the frame.
Once you add the pseudo force, you can apply <b>ΣF_real + F_pseudo = ma_relative</b> as if it were an inertial frame.
...
*   **Lift Accelerating Upwards:** In the lift's frame, the forces are N (up), mg (down), and F_p = ma (down). For equilibrium in this frame, <b>N = mg + ma = m(g+a)</b>. The person feels heavier (தோற்ற எடை அதிகம்).
*   **Lift Accelerating Downwards:** The pseudo force <b>F_p = ma</b> is now upwards. The forces are N (up), F_p (up), and mg (down). For equilibrium, <b>N + ma = mg</b>, so <b>N = m(g-a)</b>. The person feels lighter (தோற்ற எடை குறைவு).
`,
    workedExamples: [
        {
