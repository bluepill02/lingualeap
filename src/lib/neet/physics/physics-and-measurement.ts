...
*   **Units of measurements:** A standard magnitude of a quantity used to measure other quantities of the same kind.
*   **System of Units:** A complete set of units, both fundamental and derived, for all kinds of physical quantities. The common systems are FPS, CGS, MKS, and SI.
*   **S I Units:** The International System of Units. It contains 7 fundamental units (meter, kilogram, second, ampere, kelvin, mole, candela) and 2 supplementary units (radian, steradian).
*   **Fundamental and Derived Units:** Fundamental units are the units of fundamental quantities (e.g., Length, Mass, Time). Derived units are combinations of fundamental units (e.g., Velocity (m/s), Force (kg⋅m/s²)).
*   **Least Count (மீச்சிற்றளவு):** The smallest value that can be measured by a measuring instrument. (e.g., for Vernier Calipers, Screw Gauge).
*   **Significant Figures (முக்கிய எண்ணுருக்கள்):** The reliable digits plus the first uncertain digit in a measurement. There are specific rules for determining significant figures in calculations.
*   **Errors in measurements (அளவீட்டுப் பிழைகள்):** The uncertainty in a measurement. Includes Systematic Errors and Random Errors.
*   **Dimensions of Physics quantities (பரிமாணங்கள்):** The powers to which the fundamental units are raised to represent a physical quantity.
*   **Dimensional Analysis and its applications (பரிமாணப் பகுப்பாய்வு):**
    1.  To check the dimensional correctness of an equation (Principle of Homogeneity).
    2.  To derive the relationship among physical quantities.
