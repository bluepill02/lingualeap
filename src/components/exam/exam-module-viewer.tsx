...
export function ExamModuleViewer({ module }: { module: ExamModule }) {
  ...
  return (
    <div className="container mx-auto space-y-8">
       <Link href="/exam-prep" className="inline-block mb-4 text-sm text-muted-foreground hover:text-foreground">&larr; Back to All Modules</Link>
      <header>
      ...
      <Card>
        <CardHeader className="card-padding-lg">
          ...
          <CardDescription>Key terms for the {module.examName} exam</CardDescription>
        </CardHeader>
            ...
            <TableBody>
              {module.vocabulary.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {item.sentence}
                    {item.sentenceRoman && <span className="block text-sm text-muted-foreground">({item.sentenceRoman})</span>}
              ...
              <div className="space-y-2">
                {(quiz.options ?? []).map((option) => (
                  <Button
                    ...
                    variant={
                      submitted 
                        ? (option === quiz.answer ? 'success' : (answers[index] === option ? 'destructive' : 'outline'))
