...
import { getTnpscModuleById, type TnpscModule } from '@/lib/exam-data-tnpsc';
import { notFound, useRouter } from 'next/navigation';
import { MarkdownRenderer } from '@/components/exam/markdown-renderer';


export default function TnpscContentViewer({ params }: { params: { moduleId: string }}) {
  const router = useRouter();
  const { moduleId } = params;

  const [module, setModule] = useState<TnpscModule | null>(null);
  ...
  useEffect(() => {
    const moduleData = getTnpscModuleById(moduleId);
    if (moduleData) {
